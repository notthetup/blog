...
What's this 3G/4G that we keep [hearing about everywhere](http://www.tech65.org/2010/03/24/65bits-161-repeat-after-me-windows-phone-7-series/), what does that mean to me?
...
**1G.** When mobile telecommunication started off, it was analogue. All communication between the phone and the tower was just a analogue modulated signal of the speech. This is called 1G. The standards used during these times were, [NMT](http://en.wikipedia.org/wiki/Nordic_Mobile_Telephone) \* [AMPS](http://en.wikipedia.org/wiki/Advanced_Mobile_Phone_System) \* [ETACS](http://en.wikipedia.org/wiki/ETACS) etc.

**2G.** With the digital age, this communication also became digital. The voice was digitized and transmitted to the tower as a stream of '1's and '0's .The main advantages of the 2G systems were that they were encrypted ([kinda](http://revision3.com/hak5/shmoocon2010)) so you couldn't listen to other people's conversations by just catching their signals using an antenna. The digital systems were more efficient (allowed more people to talk at the same time), and they were extensible (we'll come back to this). Some of the famous 2G standards are GSM, CDMA(IS-95), iDEN. GSM was a part of a standards group called 3GPP and CDMA was part of something called 3GPP2 (Yes, I know it's not exactly this, but this makes it easier.)
...
**3G.** This was the 3rd iteration of the mobile telecommunication standards. Now this is where it get's messy. To have any standard qualify to be '3G', it had to pass some regulations set by the [ITU](http://en.wikipedia.org/wiki/International_Telecommunication_Union) (International Telecommunication Union). But that's ofcourse not how the telco advertise it.
...
**3.5G** Ofcourse, we can't stop at 3G, so there was enhancement to the standards. The 3GPP moved to HSPA(HSDPA and HSUPA), while 3GPP2 side went with EV-DO RevA. These were mainly just speed bumps. Confused? The guys at commandN did a [nice table for everything until 3.5G](http://commandn.typepad.com/cN156-techTIPS-CellGenerations.pdf).
...
[WiMAX](http://en.wikipedia.org/wiki/WiMAX) is an IEEE standard. The current version [IEEE 802.16e-2005](http://en.wikipedia.org/wiki/IEEE_802.16e-2005) can't be officially called 3G, but it's just one step before the next version [IEEE 802.16m](http://en.wikipedia.org/wiki/IEEE_802.16m) which should be called 4G.

[LTE](http://en.wikipedia.org/wiki/3GPP_Long_Term_Evolution) (Long Term Evolution) is the 3GPP groups version of next generation standard. Once again it can't perform to the requirements of ITU and hence it's considered 3.9G. The next version LTE-Advanced should be called 4G.
